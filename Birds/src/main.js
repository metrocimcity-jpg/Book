...
async function boot() {
  ...
  let credits = {};
  try {
    tree = await loadJson("data/birds.json");
  } catch (err) {
    showStatus("Data not built yet. Run npm run data in the Birds folder.");
    console.info("birds.json missing or unreadable", err);
    return;
  }
  ...
  console.info(`Loaded Aves: ${totalSpecies} species, ${(tree.children || []).length} orders`);
  showStatus("");

  const panel = createPanel($("#panel"), { credits, meta, totalSpecies });
  if (meta) {
    $("#citation").textContent = `${meta.source || "AviList: The Global Avian Checklist"} ${meta.version || ""} · ${meta.doi || ""} · retrieved ${meta.retrievedAt || ""}`;
  }
  ...
  $("#toggle-species").addEventListener("change", async (event) => {
    ...
    if (showSpecies && !speciesTree) {
      try { speciesTree = await loadJson("data/birds.species.json"); } catch { speciesTree = null; }
    }
