...
const ROOT_RANKS = new Set(["class", "group", "kingdom", "phylum"]);

export function pathNames(path) {
  return path.filter((n) => n.rank && !ROOT_RANKS.has(n.rank)).map((n) => n.name);
}
...
export function countsUnder(node) {
  ...
  if (node.rank === "family") {
    families += 1;
    if (!node.children?.length) species += node.value ?? 0;
  }
  if (node.rank === "genus") {
    genera += 1;
  ...
  if (node.children) {
    copy.children = node.children
      ...
      .filter((c) => {
        ...
        if (c.rank === "family" && !(c.children || []).length) return (c.value ?? 0) > 0;
...
export function expandSpeciesUnder(node, focusNames = []) {
  ...
  const expand = (n, lineage) => {
    const copy = { ...n };
    const nextLine = n.rank && !ROOT_RANKS.has(n.rank) ? [...lineage, n.name] : lineage;
...
export function flattenIndex(root, vernacular = {}) {
  ...
  const walkIndex = (node, lineage) => {
    const path = node.rank && !ROOT_RANKS.has(node.rank) ? [...lineage, node.name] : lineage;
    items.push({
      name: node.name,
      common: node.common || "",
      rank: node.rank,
