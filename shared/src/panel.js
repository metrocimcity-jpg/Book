...
export function createPanel(root, {
  ...
  namesLang = "la",
  noun = "taxa",
  sourceFallback = "Taxonomy source",
  sourceLink = null,
  ...
  function empty() {
    ...
    body.append(
      ...
      meta?.note && !meta.doi ? el("p", { text: meta.note }) : null,
  ...
  function citation(info) {
    const bits = [];
    const sourceLine = [info.source || sourceFallback, info.version].filter(Boolean).join(" ");
    if (sourceLine) bits.push(sourceLine + ". ");
    if (info.doi) {
      bits.push(el("a", { href: `https://doi.org/${info.doi}`, rel: "noopener noreferrer" }, info.doi));
      bits.push(". ");
    }
    if (info.retrievedAt) bits.push(` Retrieved ${info.retrievedAt}.`);
    return el("p", { class: "citation-block" }, bits);
  }

  function artwork(node) {
    const credit = creditFor(node.name);
    const src = credit?.file || placeholder;
  ...
  function links(node) {
    ...
    return el("p", { class: "ext-links" }, [
      ...
      sourceLink
        ? el("a", { class: "ext", href: sourceLink.href, rel: "noopener noreferrer", target: "_blank" }, sourceLink.label)
        : null
    ]);
  }
  ...
  function show(node, { speciesName = null } = {}) {
    ...
    body.append(
      ...
      el("div", { class: "counts" }, [
        ...
        el("div", {}, [el("b", { text: formatPercent(species, totalSpecies) }), ` of ${noun}`])
      ]),
      speciesList(node),
      links(node)
    );
